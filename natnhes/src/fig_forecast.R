...
data_direct <- readRDS("../shared/out/vals_direct_nhes.rds")

p <- ggplot(data_modelled, aes(x = year)) +
    facet_grid(rows = vars(sex), cols = vars(age)) +
    geom_ribbon(aes(ymin = y2.5, ymax = y97.5), fill = palette$quantiles[[1]]) + 
    geom_ribbon(aes(ymin = y25, ymax = y75), fill = palette$quantiles[[2]]) +
